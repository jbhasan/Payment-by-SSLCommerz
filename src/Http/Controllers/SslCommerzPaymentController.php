...
class SslCommerzPaymentController extends Controller
{
    ...
    // {
	// 	$requested_data = json_decode($request->cart_json, true);

    //     # Here you have to receive all the order data to initate the payment.
    //     # Lets your oder trnsaction informations are saving in a table called "sslcommerz_orders"
    //     # In sslcommerz_orders table order uniq identity is "transaction_id","status" field contain status of the transaction, "amount" is the order amount to be paid and "currency" is for storing Site Currency which will be checked with paid currency.

    //     $post_data = $requested_data;
    //     $post_data['total_amount'] = $requested_data['amount']; # You cant not pay less than 10
    //     $post_data['currency'] = "BDT";
    //     $post_data['tran_id'] = uniqid(); // tran_id must be unique

    //     # CUSTOMER INFORMATION
    //     // $post_data['cus_name'] = $requested_data['cus_name'];
    //     // $post_data['cus_email'] = $requested_data['cus_email'];
    //     // $post_data['cus_add1'] = $requested_data['cus_addr1'];
    //     $post_data['cus_add2'] = "";
    //     $post_data['cus_city'] = "";
    //     $post_data['cus_state'] = "";
    //     $post_data['cus_postcode'] = "";
    //     $post_data['cus_country'] = "Bangladesh";
    //     // $post_data['cus_phone'] = $requested_data['cus_phone'];
    //     $post_data['cus_fax'] = "";
    ...
    //     $update_product = DB::table('sslcommerz_orders')
    //         ->where('transaction_id', $post_data['tran_id'])
    //         ->updateOrInsert([
    //             'name' => $post_data['cus_name'],
    //             'email' => $post_data['cus_email'],
    ...
    private function fail(Request $request)
    {
        ...
        if ($order_details->status == 'Pending') {
            DB::table('sslcommerz_orders')
				...
				return ["status" => "failed", "message" => "Transaction is Falied"];
        } else if ($order_details->status == 'Processing' || $order_details->status == 'Complete') {
